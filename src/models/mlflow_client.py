...
def download_scaler(station_name, scaler_type, stage):
    ...
    try:
        ...
        latest_scaler_source = client.get_latest_versions(name=scaler_name, stages=[stage])

        print(latest_scaler_source)

        # Load the scaler by its source
        return mlflow.sklearn.load_model(latest_scaler_source[0].source)
    except IndexError:
